...
class CallMaker(StreamListener):
    #curr_loc = ''
    def on_data(self, data):
        ...
        if(tweetText1[0] == '@' + account_screen_name and 'Name:' in tweetText1[1]):
            # with open('locationPrefNew.json','a') as loc:
            #     loc.write(cpancha.curr_loc + ' ' + tweetText1[2].split()[1] + ' ' + tweetText1[3].split()[1])
            neighbor = tweetText1[4].split()[0].split('_')[1]
            #neighbor = tweetText1[1].split()[1]
            mode = tweetText1[2].split()[1]
            cpancha.neighbors[neighbor] = mode
            if(float(cpancha.location_response_counter)/5 == 0 ):
                #tweetText1[4].split()[1]
                reply = 'CALL ' + tweetText1[4].split()[0] + '_' + cpancha.call_no + ' ' + tweetText1[4].split()[1]
                twitterApi.update_status(status = reply, in_reply_to_status_id = tweetId)
                cpancha.call_no = cpancha.call_no + 1
                # print "The Call has been requested"
                # cpancha.location_response_counter = 0
                # #cpancha.flag = False
                time.sleep(5)
                # execfile('CallMaker.py')
